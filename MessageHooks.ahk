...
{
	global MessageHookQueue
	ListLines, Off
	MessageHookQueue.Push({event: event, hwnd: hwnd, idObject: idObject, idChild: idChild})
	SetTimer, ProcessMessageHookQueue, -0
	ListLInes, On
}
...
{
	global ShellMessageQueue
	WasCritical := A_IsCritical
	Critical
	ListLines, Off
	ShellMessageQueue.Push({wParam: wParam, lParam: lParam, Msg: Msg})
	SetTimer, ProcessShellMessageQueue, -0
	ListLines, On
	if(!WasCritical)
		Critical, Off
}
